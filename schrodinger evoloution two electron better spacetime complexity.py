...
L = 4e-9  # length of the simulation box (4 nm)
...
# Vectorize the potential energy function
potential_energy_vec = np.vectorize(potential_energy)
...
def time_evolution(psi, H, V, dt):
    ...
    print("Reshaped")
    print(f'psi matrix dimensions: {np.shape(psi)}')
    print(f'Hamiltonian dimensions: {np.shape(H)}')
    print(F' Potential dimensions: {np.shape(V)}')
    A = (np.eye(N**2) - 1j * dt / (2 * hbar) * (H + np.kron(V_reshaped, np.eye(N)) + np.kron(np.eye(N), V_reshaped)))
    print("got A")
    B = (np.eye(N**2) + 1j * dt / (2 * hbar) * (H + np.kron(V_reshaped, np.eye(N)) + np.kron(np.eye(N), V_reshaped)))
    print("got A,B")
    return np.linalg.solve(A, B.dot(psi.reshape(N**2))).reshape(N, N)

# Create the directory to save frames
output_dir = 'frames_trial_ivelostmymind'
...
def animate(step):
    global psi1, psi2
    # Calculate the potential energy on the fly
    V1 = potential_energy_vec(X, Y, X, Y)
    V2 = potential_energy_vec(X, Y, X, Y)
    print("got potential energy vecrtors")
    # Update the wave functions
    psi1 = time_evolution(psi1, H1, V1, dt)
    print("solved psi1")
    psi2 = time_evolution(psi2, H2, V2, dt)
    print("solved")
