...
xmax = 15.0     # Maximum x value for plotting
ymax = 150     # Maximum y value for plotting
dt = 1   # Time step
...
# Function to calculate the force from the Coulomb potential
def coulomb_force(x, y, y_shift):
    r = np.sqrt(x**2 + (y - y_shift)**2)
    fx = -x / r**3
    fy = -(y - y_shift) / r**3
    return fx, fy
...
for t in np.arange(0, total_time, dt):
    ...
    Fx, Fy = coulomb_force(X, Y, y_shift)
    Force = np.sqrt(Fx**2 + Fy**2)
    
    # Normalize the force to match the scale of the probability density
    Force_normalized = Force / np.max(Force) * np.max(np.abs(psi)**2)
    ...
    # Plot the probability density and normalized force as a 3D surface plot
    ax.clear()
    ax.plot_surface(X, Y, probability_density, cmap='viridis', edgecolor='none')
    ax.plot_surface(X, Y, Force_normalized, cmap='hot', edgecolor='none', alpha=0.5)
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.set_zlabel('Probability Density / Normalized Force')
    ax.set_title(f'Time = {t:.2f}')
    ax.set_xlim(-xmax, xmax)
    ax.set_ylim(-ymax, ymax)
    ax.set_zlim(0, np.max(probability_density))  # Adjust z-limit as needed
